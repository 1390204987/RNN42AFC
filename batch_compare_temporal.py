...
def plot_combined_temporal(ax, time_points, temp_data1, shu_data1, label1, temp_data2, shu_data2, label2, title, f1,f2):
    ...
    # 绘制File1的数据 - 实线
    ax.plot(time_points, temp_mean1, label=f'{label1} Temporal', color=f1['color'], linewidth=2)
    ...
    # 绘制File2的数据 - 实线
    ax.plot(time_points, temp_mean2, label=f'{label2} Temporal', color=f2['color'], linewidth=4)
    ...
    if sig_points1:
        ...
        ax.scatter(sig_points1, [sig_y] * len(sig_points1), 
                  color=f1['color'], marker='s', s=25, label=f'{label1} sig (p<0.05)', alpha=1)
    ...
    if sig_points2:
        ...
        ax.scatter(sig_points2, [sig_y] * len(sig_points2), 
                  color=f1['color'], marker='^', s=25, label=f'{label2} sig (p<0.05)', alpha=1)
...
file2 = "./sactemporal_batch2cut.xlsx"  # 第二个Excel文件，替换为实际路径
