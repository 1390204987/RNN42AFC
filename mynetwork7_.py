# -*- coding: utf-8 -*-
"""
Created on Thu Mar 13 15:16:41 2025
...
高斯分布衰减: 对于每个输出单元，连接强度按照高斯分布衰减,在layer2有1/4的神经元被read out。
the readin weight is fixed:分配连接: 每个输出单元与4个相邻的输入单元有较强的连接。
高斯分布衰减: 对于每个输出单元，连接强度按照高斯分布衰减。在layer1有1/4的神经元接受运动方向输入，
在layer2有1/4的神经元接受彩色目标点输入
在layer1有1/4的神经元与layer2有正向连接，1/4的神经元与layer2有反向连接
在layer2有1/4的神经元与layer1有正向连接，1/4的神经元与layer1有反向连接
@author: NaN
"""
import torch
...
class fixedReadout(nn.Module):
    ...
    def __init__(self,device,hidden_size1,hidden_size2,out_features,bias=True):
        ...
        mask21 = np.zeros((out_features,hidden_size2))
        ...
        self.weight_21 = torch.zeros((out_features,hidden_size2))
        self.weight_21[:,selected] = self.generate_fixed_weight().to(self.device)
        if bias:
    ...
    def generate_fixed_weight(self):
        ...
        fixed_weight = np.zeros((self.out_features, self.readout_size))
        connections_per_output = self.readout_size // self.out_features
        for i in range(self.out_features):
            center = i * connections_per_output + connections_per_output // 2
            for j in range(self.readout_size):
                # Gaussian-like weight centered around `center`
                distance = min(abs(j - center),self.readout_size-abs(j-center))
...
class EIRecLinear(nn.Module):
    ...
    def __init__(self, device,hidden_size1,hidden_size2,recurrency1=1,recurrency2=1,feedforward_stren=1,feedback_stren=1,bias=True):
        ...
        mask21 = np.ones((hidden_size2,hidden_size1))*feedback_stren
        mask12 = np.ones((hidden_size1,hidden_size2))*feedforward_stren
        # # genereate random index to set 1/2 hidden1 neuron and 1/2 hidden2neuron get feedforward
        # torch.manual_seed(21)
        # forward_1size = hidden_size1 // 4  # 选择 1/4 的神经元
        # selectedforward1 = torch.randperm(hidden_size1, device=device)[:forward_1size].sort().values
        # torch.manual_seed(22)        
        # forward_2size = hidden_size2 // 4  # 选择 1/4 的神经元
        # selectedforward2 = torch.randperm(hidden_size2, device=device)[:forward_2size].sort().values

        # mask21 = np.zeros((hidden_size2,hidden_size1))
        # mask21[selectedforward2, selectedforward1] = 1
        
        # # genereate random index to set 1/8 hidden1 neuron and 1/8 hidden2neuron get feedforward
        # torch.manual_seed(23)
        # back_1size = hidden_size1 // 4  # 选择 1/4 的神经元
        # selectedback1 = torch.randperm(hidden_size1, device=device)[:back_1size].sort().values
        # torch.manual_seed(24)
        # back_2size = hidden_size2 // 4  # 选择 1/4 的神经元
        # selectedback2 = torch.randperm(hidden_size2, device=device)[:back_2size].sort().values

        # mask12 = np.zeros((hidden_size1,hidden_size2))
        # mask12[selectedback1,selectedback2] = 1
        
        mask12 = mask12*feedback_stren
        mask21 = mask21*feedforward_stren
...
class ReadinLinear(nn.Module):
    ...
    def __init__(self,device,in_feature_heading,in_feature_targcolor,in_feature_rules,hidden_size1,hidden_size2,bias=True):
        ...
        # genereate random index to set 1/2 hidden1 neuron read in heading stimulus
        torch.manual_seed(31) 
        readinheading_size = hidden_size1 // 4  # 选择 1/4 的神经元
        selected_heading = torch.randperm(hidden_size1, device=device)[:readinheading_size].sort().values

        # genereate random index to set 1/4 hidden2 neuron read in T1 color
        torch.manual_seed(32)
        readinT1_size = hidden_size2 // 4  # 选择 1/4 的神经元
        selected_T1 = torch.randperm(hidden_size2, device=device)[:readinT1_size].sort().values

        # genereate random index to set 1/4 hidden2 neuron read in T2 color
        torch.manual_seed(33)
        readinT2_size = hidden_size2 // 4  # 选择 1/4 的神经元
        selected_T2 = torch.randperm(hidden_size2, device=device)[:readinT2_size].sort().values

        self.weight_11 = torch.zeros((self.hidden_size1,self.in_feature_heading-1))
        self.weight_11[selected_heading,:] = self.generate_fixed_weight(readinheading_size,self.in_feature_heading-1).to(self.device)
        self.weight_22 = torch.zeros((self.hidden_size2,int(self.in_feature_targcolor/2)))
        self.weight_22[selected_T1,:] = self.generate_fixed_weight(readinT1_size,int(self.in_feature_targcolor/2)).to(self.device) #T1 ring
        self.weight_23 = torch.zeros((self.hidden_size2,int(self.in_feature_targcolor/2)))
        self.weight_23[selected_T2,:] = self.generate_fixed_weight(readinT2_size,int(self.in_feature_targcolor/2)).to(self.device) #T2 ring
        if bias:
            self.bias = nn.Parameter(torch.Tensor(out_features)).to(device)
        else:
            self.register_parameter('bias',None)
    ...
    def effective_weight(self):
        ...
        new_weight[self.hidden_size1+2:, self.in_feature_heading:self.in_feature_heading + int(self.in_feature_targcolor/2)] = self.weight_22[0:-2,:]
