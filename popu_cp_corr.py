# -*- coding: utf-8 -*-
"""
Created on Sun Aug 10 10:27:33 2025
calculate cp 
@author: 13902
"""
...
def neuralactivity_color_dm(model_dir,device,**kwargs):
    ...
    heading_selectivity = _z_selectivity(heading_per_trial,choice_per_trial,neural_activity,times_relate)
    saccade_selectivity = _z_selectivity(sac_per_trial,heading_per_trial,neural_activity,times_relate)
    ...
    choice_selectivity = _selectivity(heading0_choice,neural_activity[:,select_0heading,:],times_relate)
    ...
    color_selectivity = _selectivity(T1loc_per_trial,neural_activity,times_relate)
    ...
    select_T1_left =  select_T10
    select_T1_right = ~select_T10
    select_choice_left = choice_per_trial==0
    select_choice_right = choice_per_trial==1
    ...
    for iheading in unique_heading:
        select = heading_per_trial==iheading
        zfir_rate[select,:] = (fir_rate[select,:]-np.mean(fir_rate[select,:],axis=0))/np.std(fir_rate[select,:],axis=0)
    fir_L_T1L = zfir_rate[select_T1_left&select_choice_left,:]
    fir_R_T1L = zfir_rate[select_T1_left&select_choice_right,:]
    fir_L_T1R = zfir_rate[select_T1_right&select_choice_left,:]
    fir_R_T1R = zfir_rate[select_T1_right&select_choice_right,:]
    
    n_units = fir_rate.shape[1]
    ROC_T1L = np.full(n_units, np.nan)
    ROC_T1R = np.full(n_units, np.nan)
    # 对每个单元分别计算ROC
    ...
    CP_T1L = np.full_like(ROC_T1L,np.nan)
    CP_T1R = np.full_like(ROC_T1R,np.nan)
    heading_sele_sign = heading_selectivity[:,0]>0 
    CP_T1L[heading_sele_sign] = 1-ROC_T1L[heading_sele_sign]
    CP_T1R[heading_sele_sign] = 1-ROC_T1R[heading_sele_sign]
    
    # 用RGB数值指定颜色 (0-1范围)
    color_rgb = [0.5, 0.5, 1]  # 示例：蓝色调
    L1_brightness = 1  # 亮度值，范围0-1
    L2_brightness = 0.5  # 亮度值，范围0-1
    # 直接乘以亮度值
    L1color = [c * L1_brightness for c in color_rgb]
    L2color = [c * L2_brightness for c in color_rgb]
    
    plt.figure()
    figname = 'cpcorr'    
    plt.scatter(CP_T1L[:hidden1_size],CP_T1R[:hidden1_size],color=L1color)
    plt.scatter(CP_T1L[hidden1_size:],CP_T1R[hidden1_size:],color=L2color)
    plt.xlim(0, 1)  # 设置x轴范围从0到1
    plt.ylim(0, 1)  # 设置y轴范围从0到1
    plt.grid(False)    
    sns.despine()  # 移除顶部和右侧的轴线
    # 添加x=0.5和y=0.5的黑色参考虚线
    plt.axvline(x=0.5, color='black', linestyle='--', linewidth=1)  # x=0.5的黑色虚线
    plt.axhline(y=0.5, color='black', linestyle='--', linewidth=1)  # y=0.5的黑色虚线
    
    # 分别计算两组数据的相关系数（忽略NaN值）
    results = []
    
    for i, (x_data, y_data) in enumerate([(CP_T1L[:hidden1_size], CP_T1R[:hidden1_size]), 
                                         (CP_T1L[hidden1_size:], CP_T1R[hidden1_size:])]):
        # 移除NaN值
        ...
        if len(x_clean) > 1:
            ...
            corr_text = f"r{i+1} = {corr:.2e}".replace('e-0', 'e-').replace('e+0', 'e+')
            p_text = f"p{i+1} = {p_value:.2e}".replace('e-0', 'e-').replace('e+0', 'e+')
            results.append(f"{corr_text}\n{p_text}")
        else:
            results.append(f"Group {i+1}: Insufficient data")
    ...
    plt.savefig("./lunwenfigure/CP_corr.svg")
    # plt.figure()
    # figname = 'cpcorr'
    # plt.scatter(CP_T1L[hidden1_size:],CP_T1R[hidden1_size:],color=L2color)
    # # plot_selectivity_corr(CP_T1L,CP_T1R,rule_name,figname,**kwargs)
    # plt.xlim(0, 1)  # 设置x轴范围从0到1
    # plt.ylim(0, 1)  # 设置y轴范围从0到1
