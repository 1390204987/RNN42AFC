...
# 
    ...
    # L1_choicetime = plot_divergence(heading0_choice,heading0_h1_activity,times_relate,
...
for net in nets:
    ...
    L1_sactime,L2_sactime,L1_choicetime,L2_choicetime = neuralactivity_color_dm(model_dir,device)
    numbersinnetname = re.findall(r'\d+',net)
    netid = numbersinnetname[0]
    
    # Replace the append code with:
    new_row = pd.DataFrame({
        "netid": [netid],
        "L1_sactime": [L1_sactime],
        "L2_sactime": [L2_sactime],
        "L1_choicetime": [L1_choicetime],
        "L2_choicetime": [L2_choicetime]
    })
    
    df = pd.concat([df, new_row], ignore_index=True)
df.to_excel("./choice_divergence_color2h1.xlsx",index=False)
