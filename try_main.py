# -*- coding: utf-8 -*-
"""
Created on Thu Jul  8 21:49:49 2021

@author: NaN
"""
#testing(learning) EI RNN
...
from try_EIRNN import EIRNN
from try_EIRecLinear import EIRecLinear
from try_net import Net
...
mean_stim0correct_response = np.mean(stim0correct_response,axis=0)
mean_stim0error_response = np.mean(stim0error_response,axis=0)
mean_stim1corect_response = np.mean(stim1corect_response,axis=0)
mean_stim1error_response = np.mean(stim1error_response,axis=0)

std_stim0correct_response = np.std(stim0correct_response,axis=0)
std_stim0error_response = np.std(stim0error_response,axis=0)
std_stim1corect_response = np.std(stim1corect_response,axis=0)
std_stim1error_response = np.std(stim1error_response,axis=0)

neuron = 6
t_range = range(25)
plt.figure()
_ = plt.plot(t_range, mean_stim0correct_response[:,neuron], color='blue', label='0')                        
_ = plt.plot(t_range, mean_stim0correct_response[:,neuron]+std_stim0correct_response[:,neuron], color='blue', alpha=0.8, label='0') 
_ = plt.plot(t_range, mean_stim0correct_response[:,neuron]-std_stim0correct_response[:,neuron], color='blue', alpha=0.8, label='0') 
_ = plt.plot(t_range, mean_stim1error_response[:,neuron], color='red', label='1') 
_ = plt.plot(t_range, mean_stim1error_response[:,neuron]+std_stim1error_response[:,neuron], color='red', alpha=0.8, label='1') 
_ = plt.plot(t_range, mean_stim1error_response[:,neuron]-std_stim1error_response[:,neuron], color='red', alpha=0.8, label='1') 
plt.xlabel('Time step')
plt.ylabel('Activity')   
